...
Each request passes through up to four enrichment stages: client IP from HTTP → Infoblox IPAM → NAC / Extreme XMC → building lookup via NIT. A full description of each stage, the fallback logic between them, and a field-by-field source table is in **[docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)**.
...
See **[DOCKER.md](docs/DOCKER.md)** for the complete step-by-step guide.
...
See **[OPENSHIFT.md](docs/OPENSHIFT.md)** for the complete step-by-step guide including S2I build setup, secret creation, PVC provisioning, and day-to-day operations.
